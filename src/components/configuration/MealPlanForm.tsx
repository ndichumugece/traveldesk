...
interface MealPlanFormProps {
    existingMealPlan?: MealPlan | null;
    onSave: (mealPlan: Omit<MealPlan, 'id' | 'created_at' | 'updated_at'>) => Promise<any>;
    onUpdate: (id: string, updates: Partial<MealPlan>) => Promise<any>;
    onDiscard: () => void;
    onDelete?: () => void;
}

export function MealPlanForm({ existingMealPlan, onSave, onUpdate, onDiscard, onDelete }: MealPlanFormProps) {
    const [name, setName] = useState(existingMealPlan?.name || '');
    const [status, setStatus] = useState<'active' | 'inactive'>(existingMealPlan?.status || 'active');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (existingMealPlan) {
                const { error: updateError } = await onUpdate(existingMealPlan.id, { name, status });
                if (updateError) throw new Error(updateError);
            } else {
                const { error: saveError } = await onSave({ name, status });
                if (saveError) throw new Error(saveError);
            }
            if (!existingMealPlan) onDiscard();
        } catch (err: any) {
            let errorMessage = err.message;
            if (errorMessage?.includes('duplicate key value violates unique constraint "meal_plans_name_key"')) {
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
            <div className="space-y-1">
                <h2 className="text-2xl font-bold text-slate-900">
                    {existingMealPlan ? 'Edit Meal Plan' : 'Create New Meal Plan'}
                </h2>
                <p className="text-slate-500 font-medium">
                    {existingMealPlan ? 'Update the details of this meal plan.' : 'Add a new meal plan to the list.'}
                </p>
            </div>
            ...
            <div className="space-y-8">
                ...
                <div className="flex items-center justify-between pt-8 border-t border-slate-100">
                    ...
                    >
                        ...
                        {existingMealPlan ? 'Update Item' : 'Create Item'}
                    </button>
                </div>
            </div>
        </form>
