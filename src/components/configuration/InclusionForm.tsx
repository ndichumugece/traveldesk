...
import { type Inclusion } from '../../hooks/useInclusions';

interface InclusionFormProps {
    existingInclusion?: Inclusion | null;
    onSave: (inclusion: Omit<Inclusion, 'id' | 'created_at' | 'updated_at'>) => Promise<any>;
    onUpdate: (id: string, updates: Partial<Inclusion>) => Promise<any>;
    onDiscard: () => void;
    onDelete?: () => void;
}

export function InclusionForm({ existingInclusion, onSave, onUpdate, onDiscard, onDelete }: InclusionFormProps) {
    const [name, setName] = useState(existingInclusion?.name || '');
    const [status, setStatus] = useState<'active' | 'inactive'>(existingInclusion?.status || 'active');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (existingInclusion) {
                const { error: updateError } = await onUpdate(existingInclusion.id, { name, status });
                if (updateError) throw new Error(updateError);
            } else {
                const { error: saveError } = await onSave({ name, status });
                if (saveError) throw new Error(saveError);
            }
            // Success! No need to close if we're in the split view, but we can reset isAdding
            if (!existingInclusion) onDiscard();
        } catch (err: any) {
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
            <div className="space-y-1">
                <h2 className="text-2xl font-bold text-slate-900">
                    {existingInclusion ? 'Edit Inclusion' : 'Create New Item'}
                </h2>
                <p className="text-slate-500 font-medium">
                    {existingInclusion ? 'Update the details of this inclusion.' : 'Add a new inclusion to the list.'}
                </p>
            </div>
            ...
            <div className="space-y-8">
                ...
                <div className="flex items-center justify-between pt-8 border-t border-slate-100">
                    ...
                    >
                        ...
                        {existingInclusion ? 'Update Item' : 'Create Item'}
                    </button>
                </div>
            </div>
        </form>
