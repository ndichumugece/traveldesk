...
interface ExclusionListProps {
    ...
    onSelect: (exclusion: Exclusion) => void;
    onAddNew: () => void;
}

export function ExclusionList({ exclusions, loading, selectedId, onSelect, onAddNew }: ExclusionListProps) {
                ...
                {loading ? (
                    ...
                    <div className="space-y-1">
                        {filteredExclusions.map((exclusion) => {
                            const isSelected = selectedId === exclusion.id;
                            return (
                                <button
                                    key={exclusion.id}
                                    onClick={() => onSelect(exclusion)}
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${isSelected ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-400 group-hover:bg-rose-50 group-hover:text-rose-500'
                                            } transition-colors`}>
                                            <XCircle size={16} />
                                        </div>
                        ...
                        {filteredExclusions.length === 0 && (
                            <div className="px-4 py-8 text-center">
                                <p className="text-xs font-medium text-slate-400">
                                    {search ? 'No matches found' : 'No exclusions yet'}
                                </p>
                            </div>
                        )}
                    </div>
