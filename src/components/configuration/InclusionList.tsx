...
interface InclusionListProps {
    ...
    onSelect: (inclusion: Inclusion) => void;
    onAddNew: () => void;
}

export function InclusionList({ inclusions, loading, selectedId, onSelect, onAddNew }: InclusionListProps) {
                ...
                {loading ? (
                    ...
                    <div className="space-y-1">
                        {filteredInclusions.map((inclusion) => {
                            const isSelected = selectedId === inclusion.id;
                            return (
                                <button
                                    key={inclusion.id}
                                    onClick={() => onSelect(inclusion)}
                        ...
                        {filteredInclusions.length === 0 && (
                            <div className="px-4 py-8 text-center">
                                <p className="text-xs font-medium text-slate-400">
                                    {search ? 'No matches found' : 'No inclusions yet'}
                                </p>
                            </div>
                        )}
                    </div>
